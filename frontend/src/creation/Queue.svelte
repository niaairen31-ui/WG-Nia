<script>
  /* TICKET-0059 (BRIEF-0059-k, commits 2-3). Faithful port of loadQueue's
     ...
     mutation is QueueCard.svelte's own concern; the batch bar
     (#creation-shell-batch-bar, a separate container) is
     QueueBatchBar.svelte's -- only the verdict banner it writes to
     queueState.batchVerdict renders here, in its original position
     between the panel head and the card list.
...
<div class="queue-panel">
  ...
  {#if queueState.batchVerdict}
    <div class="batch-verdict {queueState.batchVerdict.cls ? 'r-' + queueState.batchVerdict.cls : ''}">{queueState.batchVerdict.msg}</div>
  {/if}
