...
TICKET-0072, decisions E1 / F1 / H1. **Supersedes BRIEF-0072-c, which was never
executed** (which in turn superseded BRIEF-0072-b, also never executed). Neither
predecessor is edited; both stay on disk as the withdrawn chain. This brief is
BRIEF-0072-c with one change: `_join_gathering`'s signature is no longer touched.
One commit for both code sites plus the check.
...
committed, fails instantly with `SQLITE_BUSY_SNAPSHOT` — the same
`database is locked` message, immune to `busy_timeout`.
...
- `play_stream.py:110` — `_perform_travel(..., ctx.db)`, committing at
  ...
  (`play_initiative.py:203`, `:207`, `:250`). Crashes as soon as WAL lands.
  Measured: fails at 0.00s; succeeds on its own session.
- `play.py:390` — `_join_gathering(ctx.conv, resolved_id, ctx.db)`, committing at
  `play.py:911-913`. Measured live post-BRIEF-0072-a: a join turn completes
  cleanly today, because nothing commits between the pin at `play.py:143` and
  `play.py:686` — `play.py:266` is the only candidate and it aborts the turn
  rather than reaching the mode dispatch. Latent, not live: correct today by
  phase ordering, which is not a guarantee.

**Why the signature is left alone (H1).** BRIEF-0072-c would have changed
`_join_gathering(conv: Conversation, ...)` to take a `conv_id`, so it could
never be handed an object belonging to another session. The RECON that followed
found the function has **three** callers repo-wide — `play.py:390`,
`routes/scene.py:303`, `routes/play.py:239` — one of which pre-`flush()`es a
still-pending `Conversation` to obtain its id (`routes/scene.py:300`). That
turned a crash fix into a change touching two working HTTP endpoints. It also
made the fix depend on an exhaustive caller enumeration, and on this ticket
enumeration scope has been wrong three times. So the stream call site owns the
session boundary instead, using the idiom already established at
`play_physical.py:324-330`: open a nested session, re-fetch the row on it, work
on that copy. One site changes. The two route callers are not touched, cannot
regress, and do not need to be enumerated correctly for this fix to be correct.

## Mini-RECON — verify before writing

Most of this was measured in the RECONs of 2026-08-22. Re-verify the anchors
...
2. `play.py:390` and `play_stream.py:110` are still the ONLY one-hop call sites
   ...
   `routes/scene.py` callers. If a third stream-reachable site appeared, STOP.
3. `_join_gathering`'s signature at `play.py:890` is unchanged and its three
   callers are unchanged. **Enumerate repo-wide** — `grep -rn "_join_gathering"
   --include=*.py .` — and paste the output verbatim with the count, rather than
   asserting a number. Expected: definition at `play.py:890`, calls at
   `play.py:390`, `routes/scene.py:303`, `routes/play.py:239`, plus the two
   import lines (`routes/scene.py:37`, `routes/play.py:51`). A different set is
   not automatically a STOP under this brief — the two route callers are out of
   scope either way — but report it.
4. `Session` and `engine` are already imported at module level in `play.py`
   (`from ..db import engine`, `play.py:33`) and in `play_stream.py` (used at
   `play_stream.py:116`), and `Conversation` is imported in `play.py`. Report if
   any new import is needed.
5. `_SayAbort` (`play.py:77-84`) still carries `sse_lines: list[str]` and is
   still caught at `play.py:718-719`, so a phase helper raising it ends the turn
   with its lines flushed before `[DONE]`.
6. The alias list is 8, not 6: `play.py:493`, `play.py:554`,
...
7. Zero attribute assignments of the form `ctx.conv.<attr> = ...` exist in the
   four Play modules today. Re-measure. If any appeared, STOP — rule4 of the
   check would ship red and that site needs its own analysis.
8. `analyzer.py` and `gathering.py` contain writes and are called from the
...
1. **`src/world_engine/cockpit/play.py` — the join call site.** Replace
   `play.py:390-391` (the `_join_gathering` call and the `extra_event` line that
   consumes it) with:

   ```python
           # TICKET-0072 (BRIEF-0072-d, E1/H1). Its own session, deliberately.
           ...
           # ordering, not a guarantee. _join_gathering keeps its signature and
           # its two other callers: the session boundary is owned here, by
           # re-fetching the row on the nested session, as at
           # play_physical.py:324-330.
   ...
   - `joined_id` / `joined_label` are read **inside** the `with` block. The
     returned `Gathering` belongs to `join_db`; once that session closes the
     object is detached and nothing may be read off it. Plain values cross the
     boundary, ORM objects do not.
   - The `None` branch raises `_SayAbort` rather than following the
     `if ss_conv:` skip idiom of `play_physical.py:326`. Skipping a scene-state
     write degrades a turn; silently skipping a join would report a join that
     never happened. Fail-closed, and the turn ends with the error visible to
     the client.
   - **`ctx.conv` is NOT mutated.** The RECON measured `autoflush = True` on the
     request session, so assigning `ctx.conv.gathering_id` in memory would emit
     an UPDATE into the pinned transaction at the next query on `ctx.db` — a
     write on the request session with no call site, invisible to rule1 and
     rule2, which FastAPI's dependency teardown then rolls back. That is the
     exact class this step removes, arriving through the back door. The RECON
     also established there is no consumer: `player_gathering` is computed once
     at `play.py:677` and threaded unchanged into `_say_initiative_phase`, so on
     a first-time join it is `None`, initiative returns early at
     `play_initiative.py:61-62`, and `play_initiative.py:112` is never reached.
     No structure without a reader.

2. **`src/world_engine/cockpit/play_stream.py` — the travel transition.**
   Replace the body of the `if travel_dest_id is not None:` block
   (`play_stream.py:108-110`) with:

   ```python
   if travel_dest_id is not None:
       # TICKET-0072 (BRIEF-0072-d, E1). Its own session, for the same reason
...
3. **`tooling/verify/checks/stream_session_readonly.py`** — new fail-closed
   ...
   **Definitions**, computed in this order:
   ...
   - **WRITERS** — every function defined anywhere in the declared set whose
     ...
     shape this exists for; `analyzer.py` and `gathering.py` are why the set
     reaches past the four Play modules.
   ...
   - **rule1 — no direct write.** Any
     `<req>.add/.delete/.merge/.commit/.flush(...)` where `<req>` is a
     request-session expression is a FAILURE, naming `file:line`.
   - **rule2 — no write by delegation.** Any call site in the four Play modules
     ...
     naming `file:line` and the callee. This catches both `play.py:390` and
   ...
   - **rule3 — unresolvable callee is a failure, not a pass.** If a
     ...
     grows deliberately, in a ticket. Fail-closed on the unknown is the point:
     the alternative is a green that means "I could not see."
   - **rule4 — no attribute assignment on the request session's conversation.**
     ...
     no call site. Measured zero occurrences today, so this rule ships green and
   ...
   - **rule5 — vacuity guards, four of them.** Every declared module must exist
     ...
     count of call sites passing a request-session expression to any callee must
     be non-zero. Each empty result means the walker stopped seeing the code,
     not that the code is clean. Each is a FAILURE.
   ...
   rule3 as a failure, never waved through.** The check does not resolve the
   whole program — it refuses to guess about what it cannot see.
...
4. **`ARCHITECTURE_DECISIONS.md`** — append to the entry BRIEF-0072-a opened; do
   not rewrite it. Record:
   - the request session is read-only for the life of a streaming response;
   - both halves of TICKET-0072 are load-bearing and neither suffices alone —
     WAL alone relocates the crash from dialogue turns to travel turns, and the
     session change alone leaves failure mode 1 intact;
   - the autoflush finding: an in-memory mutation on a session-attached object
     is an invisible write, forbidden by rule4;
   - **the enumeration-scope rule.** Three enumerations on this ticket were
     scoped to the four Play modules when the artifact's blast radius was wider,
     and each miss produced a STOP. Any brief that changes a shared function's
     signature carries a repo-wide enumeration requirement — grep across the
     tree, output pasted verbatim, count stated — never "confirm there is one
     caller." The corollary chosen here: **prefer the fix whose correctness does
     not depend on a complete enumeration.** H1 over the signature change was
     decided on exactly that ground.
   - the named condition under which `ctx.conv.gathering_id`'s staleness after a
     join stops being harmless: the day `player_gathering` is recomputed after
     the mode dispatch, the joined id must be threaded explicitly to the
     initiative phase.

5. **`world-engine-schema-changelog.md`** — extend the ticket's single
...
- **`_join_gathering`'s signature, body and return type** (`play.py:890-915`).
  Byte-identical after this step, including its `db.refresh(gathering)` — that
  refresh is what makes the returned object readable after the commit, and Scope
  IN item 1 depends on it. Named deferral, hardening the contract so it cannot
  be handed an object from a foreign session: **reactivation condition — a
  fourth caller appears, or a second caller needs the nested-session shape.**
- **`routes/scene.py:303` and `routes/play.py:239`.** Both call
  `_join_gathering` on a non-streaming request session with no pinned-snapshot
  problem. Untouched, unread, not refactored. `routes/scene.py:300`'s
  `db.flush()`-a-pending-`Conversation` pattern is deliberately left alone.
- **`_perform_travel` itself** (`play_stream.py:382-459`) and its three
  non-stream callers (`routes/play.py:280`, `:384`, `:454`).
- **`_scene_response` and its four callers** (`routes/scene.py:77`, `:156`,
  `:395`, `:426`).
...
- **Mutating `ctx.conv` in any way.** Forbidden by Scope IN item 1 and by rule4.
...
- **Widening the declared module set beyond Scope IN item 3.** If rule3 fires on
...
- **Fail-closed, and fail-closed on the unknown.** rule3 is the sharp edge: an
  ...
  writer set: FAILURE. In the application code, a missing `Conversation` on the
  nested session aborts the turn rather than skipping the join.
- **Vacuous-proof.** rule5's four guards. An AST walker that silently matches
  nothing is green-by-blindness, and this check's subject is a site that read as
...
- **One implementation per rule.** No overlap with `sqlite_concurrency.py` —
  ...
  from the report line alone which guarantee lapsed. And exactly one way to join
  a gathering survives this step: `_join_gathering`, called by three sites, no
  adapter, no second path.
- **History is sacred.** `_join_gathering` stays idempotent — re-joining the
  same gathering leaves the existing open `GatheringMember` row untouched rather
  than closing and reopening it. `_perform_travel` still archives `scene_state`
  to `history[]` before clearing (`play_stream.py:436`); moving it to another
  session changes which connection carries the write, nothing about what is
  written or in what order.
- **No structure without a reader.** The joined id and label are read out
  because `extra_event` consumes them. Nothing is synced onto `ctx.conv`,
  because nothing reads it.
- **No canon-write path, no schema.** Session ownership only.

## Done means

- [ ] Mini-RECON items 1-8 reported with `file:line`, including the verbatim
      repo-wide `_join_gathering` grep with its count, the resolved alias count,
      and the re-measured zero for `ctx.conv.<attr> = ...`
- [ ] `git diff` shows exactly two changed blocks in application code: the join
      call site and the travel transition. `_join_gathering`, `_perform_travel`,
      `routes/scene.py` and `routes/play.py` are byte-identical
...
- [ ] It returns FAIL when the transitive closure is temporarily disabled, i.e. WRITERS computed one-hop only, with a temporary `ctx.db` passed to `_scene_response` (demonstrate, revert) — proves the closure does work rather than decorate
...
- [ ] Live: `POST /api/scene/join` still works (the caller this brief deliberately did not touch)
- [ ] Live: `POST /api/conversations/{conv_id}/join` still works (same)
...
- `ARCHITECTURE_DECISIONS.md` — append to BRIEF-0072-a's entry (Scope IN item 4),
  including the enumeration-scope rule.
- `world-engine-schema-changelog.md` — extend the ticket's single applicatif
  addendum entry (Scope IN item 5).
