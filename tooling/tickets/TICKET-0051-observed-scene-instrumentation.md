...
- [ ] The Observation surface is a top-level mode-tab with no `TAB_KEYS`/`CREATION_TABS` leak, its transcript renderer visually distinguishes all four beat outcomes (`degraded` never resolving to the same CSS class as `silence`), its run-detail renderer reads every pinned arbitration parameter and per-usage template id/version, and no route in the observation route module writes an `observation_*` row directly  -> verify/checks/observation_surface.py
- [ ] `scripts/observation_metrics.py` performs no write operation and imports no local-model client, an NPC-captured fixture yields entropy near 0 and an even fixture near 1, a `degraded` fixture reports a non-zero degraded rate with the suspect-run warning printed, and the script's `not_selected_reason` precedence is the shared `observation_reads.derive_not_selected_reason` (never a local reimplementation)  -> verify/checks/observation_metrics.py
