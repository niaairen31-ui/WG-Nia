"""G1 check: NPC group agent staging strata (TICKET-0037, BRIEF-0037-a).
...
Two structural guarantees, both fail-closed (sibling of
`link_agent_strata.py`; guarantees 3-4 there — the D3 subject-stamp scan and
the no-direct-canon-write scan — have no analogue yet, since this step has
no commit path to scan. They arrive with BRIEF-0037-c's commit path):

  1. `npc_batch`/`npc_batch_row` appear in NEITHER `canon_write_policy.txt`
     NOR any `src/world_engine/writes/` module — this step adds zero canon
     write sites.
