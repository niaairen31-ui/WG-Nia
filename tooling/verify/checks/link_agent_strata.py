"""G1 check: NPC link agent staging strata (TICKET-0036, BRIEF-0036-a/b/c).
...
Four structural guarantees, all fail-closed:
  ...
  4. BRIEF-0036-c: `cockpit/routes/link_agent.py` and `link_author.py`
     ...
     coherence patch (`apply_finding`) and commit (`commit_batch`) paths
     are structurally forced through the `write_relation`/`write_knowledge`
     chokepoints, same as every other creator-direct-authority write.
...
LINK_CONTEXT_FILE = SRC / "world_engine" / "link_context.py"
LINK_AGENT_ROUTES_FILE = SRC / "world_engine" / "cockpit" / "routes" / "link_agent.py"
...
ALLOWED_REFERENCE_FILES = {
    ...
    "src/world_engine/link_context.py",
    "src/world_engine/models/ephemeral.py",
    "src/world_engine/models/__init__.py",
    "src/world_engine/cockpit/app.py",
}

# BRIEF-0036-c item 6: the only two files where the coherence patch/commit
# logic lives. No canon Relation()/Knowledge() construction or raw SQL is
# permitted here — everything routes through writes/relations.py::write_relation
# and writes/knowledge.py::write_knowledge.
DIRECT_WRITE_SCAN_FILES = {
    "src/world_engine/cockpit/routes/link_agent.py": LINK_AGENT_ROUTES_FILE,
    "src/world_engine/link_author.py": LINK_AUTHOR_FILE,
}
_CANON_MODEL_NAMES = {"Relation", "Knowledge"}
...
class _DirectWriteVisitor(ast.NodeVisitor):
    """Flags `db.add(Relation(...))` / `db.add(Knowledge(...))` and raw SQL
    `text(...)` strings that INSERT/UPDATE the relation or knowledge tables."""
    ...
    def visit_Call(self, node: ast.Call) -> None:
        if (
            ...
            self.hits.append(
                ...
                "direct canon write bypasses write_relation/write_knowledge"
            )
        if (
            ...
            if ("INSERT" in sql or "UPDATE" in sql) and ("RELATION" in sql or "KNOWLEDGE" in sql):
                self.hits.append(f"line {node.lineno}: raw SQL text(...) touching relation/knowledge")
        self.generic_visit(node)
