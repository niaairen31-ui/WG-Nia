...
## A SOCIAL RELATION HAS ONE PERCEIVER, AND ITS LIEN FACT IS ITS KNOWABLE FACE (BRIEF-0090-a, BRIEF-0090-b, schema v2.04)

**Decision (TICKET-0090, C3, O1, P1, N1, V1, W1, U3, T1).** A social
relation (any `type` outside `relation_orientation.RELATION_GRAPH_EXCLUDED_TYPES`)
is one perceiver's feeling toward one target: `direction = 'a_to_b'`,
`entity_a` feels, `entity_b` receives. `direction` is kept (O1), guarded in
`write_relation`, with no CHECK (a CHECK would need a table rebuild). A
legacy `mutual` row became two independent rows; a `b_to_a` row had its
endpoints swapped; each changed row carries its previous endpoints and
direction in `change_history` (`{"migration": "v2.04", "was": ...}`, or
`split_from` on a mirror) because `_append_history_snapshot`'s shape does not
name endpoints. At most one social row per oriented pair (P1), enforced by
the partial unique index `idx_relation_oriented_social`; its predicate
re-types the excluded set only because a SQLite index predicate cannot
import Python. Structural rows (`connects_to` keeps `mutual`; `controls`)
are untouched by the rule and by the migration.

**The lien fact is the relation's knowable face (N1, V1).** Every social
relation carries exactly one typed fact (`fact.relation_id`), content
generated by `lien_fact_content`, born `default_level = 'unaware'` -- it
renders for nobody until a knowledge row or a scoped default names a
knower. `notes` stays on the relation row as the perceiver's own view,
read in play by the perceiver only. `write_relation` births the typed fact
for every relation it creates, social or `connects_to` (W1); the migration
backfilled the 2 `connects_to` edges born after v2.00 without one, with
v2.00's content and `default_level = 'knows'`, so reachability is restored,
not invented.

**`visible_to_b` retired into knowledge, split three ways (U3).** On a
directed `a_to_b` row the flag passed a review surface (RECON-0090 R-19),
so each of the 40 TRUE rows became one `knowledge` row for `entity_b` on
the lien fact (`level = 'knows'`, source `migrate_v2_04 (visible_to_b)`).
On `b_to_a` the flag names the side that already feels and no code fixed
its meaning (R-20): the 6 TRUE rows were reported by name, never converted,
left to the sheet's "X le sait" control. On `mutual` the flag carried no
information: the 67 values were dropped. The column stays for one ticket as
dead weight; no play path reads it.

**Delete cascades (T1) -- a named exception to "History is sacred".**
Deleting a relation removes its typed fact, that fact's knowledge rows and
its scoped defaults, then the relation, in one transaction, with no
history snapshot. Same family as the `skill_definition` delete
(CLAUDE.md, "A `skill_definition` delete always succeeds"): the dependents
exist only through the row being deleted, and a fact that outlives its
relation would assert something the canon no longer holds.

**Rejected.** O2 (drop `direction` for social rows) -- every perceiver
reader keys on it; O3 (CHECK) -- table rebuild. P2 (several rows per
oriented pair keyed by type) -- a delta could not choose its row. U1 (no
conversion) -- discards reviewed decisions; U2 (convert every TRUE) --
TRUE was a default on `mutual` and undefined on `b_to_a`, and would make a
PC "know" what every NPC feels toward them. T2 (refuse delete while knowers
