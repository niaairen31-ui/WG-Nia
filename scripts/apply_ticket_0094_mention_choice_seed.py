"""One-shot, idempotent delivery of the TICKET-0094 `day_mention_choice` prompt
head onto the live DB (BRIEF-0094-C).
...
(now ten heads), relying on upsert_prompt_template's idempotence (S2) so
the nine pre-existing heads report `existing` and only the tenth
(`pt-day-mention-choice`) reports `created` on a first run.
...
if _env not in ("prod", "test"):
    print(
        "apply_ticket_0094_mention_choice_seed.py refuses to run unless "
