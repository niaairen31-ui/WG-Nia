"""One-shot, idempotent delivery of the TICKET-0018 prompt onto the live DB
(BRIEF-0018-a, item 9, S2).

`pt-world-tick-events`'s head already exists since TICKET-0017 — only the
append-a-version branch is needed here (0015/0016 shape). Embeds NO prompt
...
Run order on the live DB:
    ...
    python scripts/apply_ticket_0018_prompt_updates.py    # appends v(n+1), or reports unchanged
