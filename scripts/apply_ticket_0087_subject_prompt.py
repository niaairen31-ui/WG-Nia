"""One-shot, idempotent delivery of the TICKET-0087 `conversation_analysis`
prompt update onto the live DB (BRIEF-0087-b, Scope IN item 6).

`pt-conversation-analysis` already exists — only the append-a-version branch
is needed (`apply_ticket_0078_narration_seed.py` shape). Embeds NO prompt
text of its own; it imports `CONVERSATION_ANALYSIS_SYSTEM_PROMPT`/
`CONVERSATION_ANALYSIS_USER_TEMPLATE` from `scripts/seed_pilot.py` (single
source of text). History is sacred: this NEVER edits the existing version
row in place — a changed system prompt lands as a new `prompt_version` row,
...
_HEAD_ID = "pt-conversation-analysis"
...
def main() -> None:
    with Session(engine) as session:
        ...
        system_prompt = seed_pilot.CONVERSATION_ANALYSIS_SYSTEM_PROMPT
        user_template = seed_pilot.CONVERSATION_ANALYSIS_USER_TEMPLATE
        ...
        version = write_prompt_version(
            ...
            note="TICKET-0087 BRIEF-0087-b -- adds the subject_entity_id field",
        )
        session.commit()
