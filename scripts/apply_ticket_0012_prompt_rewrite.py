"""One-shot, idempotent delivery of the TICKET-0012 prompt lean rewrite onto
the live DB (BRIEF-0012-a, item 10, G1/S2).

TICKET-0011's S2 guarantee means `seed_pilot.py` never touches text on an
existing head, so the lean rewrite must reach the live DB as new
`prompt_version` rows through `writes.write_prompt_version` — the single
sanctioned write shape. This script embeds NO prompt text of its own; it
imports the rewritten constants from `scripts/seed_pilot.py` (single source
of text) and writes only what differs from the head's current version.

Run order on the live DB:
    python scripts/seed_pilot.py                         # converges head fields
    python scripts/apply_ticket_0012_prompt_rewrite.py   # text as new versions
...
# (head_id, system_prompt constant, user_template constant) — the five heads
# touched by the lean rewrite.
TOUCHED_HEADS = [
    ...
    ("pt-mj-narration", seed_pilot.MJ_NARRATION_SYSTEM_PROMPT, seed_pilot.MJ_NARRATION_USER_TEMPLATE),
    ("pt-mj-interpretation", seed_pilot.MJ_INTERPRETATION_SYSTEM_PROMPT, seed_pilot.MJ_INTERPRETATION_USER_TEMPLATE),
    ("pt-region-manifest", seed_pilot.REGION_MANIFEST_SYSTEM_PROMPT, seed_pilot.REGION_MANIFEST_USER_TEMPLATE),
]
