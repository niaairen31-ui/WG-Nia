"""One-shot, idempotent delivery of the TICKET-0024 prompt updates onto the
live DB (BRIEF-0024-c, item 7, S2).

`pt-world-tick` already exists — only the append-a-version branch is
needed (0015/0016/0018/0019/0020 shape). Embeds NO prompt text of its own;
it imports the constants from `scripts/seed_pilot.py` (single source of
text).

Run order on the live DB:
    python scripts/seed_pilot.py                          # converges head fields (no-op on text, a version already exists, S2)
    python scripts/apply_ticket_0024_prompt_updates.py    # appends v(n+1), or reports unchanged
