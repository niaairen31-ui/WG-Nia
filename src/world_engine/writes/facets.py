...
def add_entity_fact(
    ...
    *,
    ...
    fact = create_fact(
        db, world_id=entity.world_id, content=content, created_by=created_by,
...
def _write_coutumes(db: Session, *, entity_id: str, value: Any, created_by: str) -> list[Fact]:
    ...
    for item in value:
        ...
        facts.append(add_entity_fact(
            ...
            created_by=created_by, aspect=item.get("aspect"), scope=scope,
        ))
    return facts


def _write_creator_meta(db: Session, *, entity_id: str, value: Any, created_by: str) -> list[Fact]:
...
def write_entity_facets(db: Session, *, entity_id: str, facets: dict, created_by: str) -> list[Fact]:
    ...
    skipped. Returns the created facts in input order."""
    if not isinstance(facets, dict):
        raise ValueError("facets must be an object")
    created: list[Fact] = []
    for key, value in facets.items():
        if key == CREATOR_META_KEY:
            created += _write_creator_meta(db, entity_id=entity_id, value=value, created_by=created_by)
            continue
        ...
        if value is None:
            continue
        if key == "coutume":
            created += _write_coutumes(db, entity_id=entity_id, value=value, created_by=created_by)
...
def edit_entity_fact(db: Session, *, fact_id: str, content: str, changed_by: str) -> Fact:
    ...
    if not isinstance(content, str) or not content.strip():
        raise ValueError("fact content is empty")
    return update_fact_content(db, fact=fact, content=content, changed_by=changed_by)
